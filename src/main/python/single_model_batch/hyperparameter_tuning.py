...
mean = np.asscalar(np.load(root_path + '{}_mean.npy'.format(pathology.lower())))
std = np.asscalar(np.load(root_path + '{}_std.npy'.format(pathology.lower())))

# standardize images in batches
batches = np.arange(6) * 20000
for idx1, idx2 in zip(batches[:-1:], batches[1:]):
    print(idx1, idx2)
    train_x[idx1:idx2] = (train_x[idx1:idx2] - mean)/std
    valid_x[idx1:idx2] = (valid_x[idx1:idx2] - mean)/std
