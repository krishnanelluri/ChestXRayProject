...
def load_data(root_path, branch_names, subset=0):
    ...
    for name in branch_names:
        ...
        # class_weights_branches[name][0]=1.0 #overwrite weight for uncertainty class
...
class Metrics(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.write_model:
            self.model.save("./Atelectasis_val_auc_{:0.4f}.h5".format(_valid_auc))
...
class myDenseNet:
    ...
    def _build_one_branch(inputs, branch_name, fn_act, final_act):
        x = Dense(256, activation=fn_act)(inputs)
        output = Dense(1, activation=final_act, name=branch_name)(x)
        # output = Activation(final_act, name=branch_name)(x)
        return output
...
model.fit(x=train_x, y=train_y_branches, batch_size=32, epochs=15, verbose=2,
